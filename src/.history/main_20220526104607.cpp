...
struct identifiers /*关键字*/
{
  char iden[MAXBUF];
  int token;
};
struct identifier identable[MAX];

char Identifiers[IDEN_MAX][100]={};
...
/***************对标识符进行搜索**************/
int Isindentifier(char *is_res)
{
  int i;
  for (i = 0; i < IDEN_MAX; i++)
  {
    if ((strcmp(identable[i].iden, is_res)) == 0)
      break;
  }
  if (i < IDEN_MAX)
    return identable[i].token;
  else
    return 0;
}
/*****************判断是否为字母*****************/
...
void analyse(FILE *fpin)
{
  ...
  while ((ch = fgetc(fpin)) != EOF)
  {
    ...
    else if (IsLetter(ch))
    {
      ...
      if (Iskeyword(arr))
      { /*如果是关键字*/
        std::cout << "<" << Iskeyword(arr) << " , " <<arr<<">"<<std::endl;
      }
      else if(Isindentifier){
    ...
    { 
      ...
      if (s == 0)
        {
          int number = arr[j]-'0';
          std::cout<<"<"<<2<<" , "<<std::bitset<8>(number)<< ">"<<std::endl;
        }
      else if (s == 1)
